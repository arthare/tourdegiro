  ...
  */
  ...
  switch (node->type)
  {
    case MXML_ELEMENT :
	  ...
	  nedfree(node->value.element.name);
	...
	if (node->value.element.num_attrs)
	{
	  for (i = 0; i < node->value.element.num_attrs; i ++)
	  {
	    if (node->value.element.attrs[i].name)
	      nedfree(node->value.element.attrs[i].name);
	    if (node->value.element.attrs[i].value)
	      nedfree(node->value.element.attrs[i].value);
	  }

          nedfree(node->value.element.attrs);
	}
        break;
    case MXML_INTEGER :
       /* Nothing to do */
    ...
    case MXML_TEXT :
	  ...
	  nedfree(node->value.text.string);
        break;
    case MXML_CUSTOM :
         ...
         mxml_type_t type)		/* I - Node type */
{
  ...
  */
  ...
  if ((node = nedcalloc(1, sizeof(mxml_node_t))) == NULL)
  {
#if DEBUG > 1
